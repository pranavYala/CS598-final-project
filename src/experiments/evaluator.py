...
def main():
    ...
    class NPZDataset(torch.utils.data.Dataset):
        ...
        def __getitem__(self, i):
            ...
            ts = z['data'].astype(np.float32)
            # load all binary-task labels by their abbrev:
            labels = {
                'MOR': z['labels_MOR'].astype(np.float32),
                'CMO': z['labels_CMO'].astype(np.float32),
                'DNR': z['labels_DNR'].astype(np.float32),
                'ICD': z['labels_ICD'].astype(np.float32),
                'LOS': z['labels_LOS'].astype(np.float32),
                'REA': z['labels_REA'].astype(np.float32),
                'DIS': z['labels_DIS'].astype(np.float32),
                'ACU': z['labels_ACU'].astype(np.float32),
                'WBM': z['labels_WBM'].astype(np.float32),
                'FTS': z['labels_FTS'].astype(np.float32),
            }
            return ts, labels
    ...
    task_heads = {t: nn.Linear(encoder.output_dim, 1) for t in ['MOR','CMO','DNR','ICD','LOS','REA','DIS','ACU','WBM','FTS']}
